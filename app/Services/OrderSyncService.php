...
class OrderSyncService
{
    ...
    public function pushWooOrderToPOS(array $wooOrder, int $installationId): bool
    {
        ...
        Log::info("POS Scyn order Payload:", (array) $payload);
            ...
            if (!empty($item['meta_data'])) {
                foreach ($item['meta_data'] as $meta) {
                    if (isset($meta['key'])) {
                        ...
                        if ($key === 'size' || $key === 'sizename') {
                            $sizeName = $meta['value'] ?? null;
                        } elseif ($key === 'color' || $key === 'colname' || $key === 'colour') {
                            $colorName = $meta['value'] ?? null;
                        }
                    }
                }
            }

            // Fallbacks to avoid null constraint errors
            if (empty($sizeName)) {
                $sizeName = 'Default';
            }
            if (empty($colorName)) {
